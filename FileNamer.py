...
"""
For MEG data, the BIDS format is structured as such:
basepath
    <name>.scans.tsv
    meg
        <name>_coordsystem.json
        <name>_channels.tsv
        <name>_meg.json
        <name>_meg.<file type>  <- actual data
"""
...
a = BIDSName(subject="matt", session="test", task="task1", run="1")
b = ['coordsystem.json','channels.tsv','meg.json','scans.tsv', '.con', 'mrk']
for i in b:
    print(a.get_filename(i))