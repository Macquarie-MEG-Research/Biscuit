...
class CheckSavePopup(simpledialog.Dialog):
    ...
    def buttonbox(self):
        ...
        w = Button(self.master, text="Save and Exit", width=15,
                   command=self.save, default=ACTIVE)
        w.grid(column=0, row=1, padx=5, pady=5)
        w = Button(self.master, text="Go back!", width=15, command=self.cancel)
        w.grid(column=1, row=1, padx=5, pady=5)
        w = Button(self.master, text="Exit", width=15, command=self.exit)
